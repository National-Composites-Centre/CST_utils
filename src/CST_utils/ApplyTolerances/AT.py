...
from CST_utils.CATIA.CATIA_utils import display_file, CATIA_ctrl
import tkinter.messagebox as msg
import itertools
import win32com.client.dynamic
from CST_utils.CATIA.vecEX3 import wrmmm
...
class ToleranceApp:
    def __init__(self, path, file, CompoST_file = None):

        if CompoST_file == None:
            D = cs.Open(file,path)
        else:
            D = CompoST_file
...
if __name__ == "__main__":
    ...
    app = ToleranceApp(args.path, args.file)
    app.run()
